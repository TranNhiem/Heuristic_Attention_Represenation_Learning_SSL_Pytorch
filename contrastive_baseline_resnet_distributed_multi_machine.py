...
parser.add_argument('--temperature', default=0.1, type=float) #simclr 0.05 best contrastvie acc [0.05, 0,5, 0,1]
...
with strategy.scope(): 
    ...
    def main(args):
        ...
        def co_distributed_loss(p, z, temperature, distribute_batch):
            per_batch_co_distribute_loss= nt_xent_asymetrize_loss_v2(p, z,temperature, per_worker_batch_size)
        ...
        lr_schedule= WarmUpAndCosineDecay(base_lr,global_batch_size, total_training_sample, scale_lr,warmup_epochs, train_epochs )
        ...
        write_checkpoint_dir = write_filepath(checkpoint_dir_encoder, task_type, task_id)

        write_checkpoint_dir_1 = write_filepath(checkpoint_dir_mlp, task_type, task_id)
